...
if [ "$method" = "POST" ] && [ "$path" = "/save" ]; then
    ...
    # Send response FIRST so the browser gets JSON immediately.
    respond "200 OK" '{"status":"ok","message":"Settings saved, server restarting..."}'

    # Fork restart into background so we can exit(0) NOW and flush stdout.
    # Docker restart can take >10s due to stop_grace_period (1m), but the
    # browser already has the JSON response — no timeout, no JSON.parse error.
    if [ -S /var/run/docker.sock ]; then
        CONTAINER="${APP_ID}_server_1"
        nohup timeout 10 sh -c 'printf "POST /containers/%s/restart HTTP/1.0\r\nHost: localhost\r\n\r\n" "$1" | socat - UNIX-CONNECT:/var/run/docker.sock' _ "$CONTAINER" >/dev/null 2>&1 &
    fi

    # Exit NOW — this flushes stdout, delivering the HTTP response.
    # The background restart continues independently.
    exit 0
